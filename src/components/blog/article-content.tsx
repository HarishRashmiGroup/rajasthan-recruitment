...
export interface ContentItem {
    id: string | number;
    type: 'paragraph' | 'heading' | 'table' | 'list' | 'faq' | 'note' | 'tips' | 'process';
    text?: string;
    ...
    const renderContent = (contentItem: ContentItem) => {
        ...
        switch (contentItem.type) {
                    ...
                    >
                        <List.Root as={ListComponent} gap={2} pl={4}>
                            {(contentItem.items as string[]).map((item, idx) => (
                                <List.Item key={idx} as={'p'} fontSize={{ base: 'sm', md: 'md', lg: 'lg' }} whiteSpace="pre-wrap" color={textColor}>
                                    {item}
                                </List.Item>
                            ))}
                        </List.Root>
            ...
            case 'faq':
                ...
                return (
                    <Box key={itemId} mb={6} as="section" aria-labelledby="faq-heading">
                        <Accordion.Root>
                            {(contentItem.items as { question: string; answer: string }[]).map((faq, idx) => (
                                <Accordion.Item
                                    key={idx}
                                    value={`faq-${idx}`}
                                    border="1px"
                                ...
                                >
                                    <Heading as="h3" fontSize={{ base: 'sm', md: 'md', lg: 'lg' }}>
                                    ...
                                    >
                                        <Accordion.ItemBody px={4} py={3}>
                                            <Text whiteSpace="pre-wrap" itemProp="text">{faq.answer}</Text>
                                        </Accordion.ItemBody>
                                    </Accordion.ItemContent>
                                </Accordion.Item>
                            ))}
            ...
            case 'note':
                ...
                return (
                    ...
                    >
                        <Alert.Indicator />
                        <Box>
                            <Alert.Title as={"h3"} fontSize={{ base: 'sm', md: 'md', lg: 'lg' }}>
                                {(contentItem.noteType || 'info').charAt(0).toUpperCase() + (contentItem.noteType || 'info').slice(1)} Note:
                            </Alert.Title>
                            <Alert.Description as={"p"} fontSize={{ base: 'sm', md: 'md', lg: 'lg' }}>
                                {contentItem.text}
                            </Alert.Description>
                        </Box>
                    </Alert.Root>
                        ...
                        <VStack align="stretch" gap={2} w="full">
                            {(contentItem.items as string[]).map((item, idx) => (
                                <HStack key={idx} align="flex-start">
                                    ...
                                    <Text as="p" fontSize={{ base: 'sm', md: 'md', lg: 'lg' }} color={textColor}>{item}</Text>
                                </HStack>
                            ))}
                        </VStack>
                    </Alert.Root>
            ...
            case 'process':
                ...
                return (
                    <VStack key={itemId} gap={4} mb={6} align="stretch">
                        {contentItem.steps.map((step, idx) => (
                            ...
                            >
                                ...
                                <Box flex={1}>
                                    <Heading as="h4" fontSize={{ base: 'sm', md: 'md', lg: 'lg' }} mb={2} color={headingColor}>
                                        {step.title}
                                    </Heading>
                                    <Text as={'p'} fontSize={{ base: 'sm', md: 'md', lg: 'lg' }} color={textColor} whiteSpace="pre-wrap">
                                        {step.description}
                                    </Text>
                                </Box>
                            </HStack>
